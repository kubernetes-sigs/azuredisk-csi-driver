...
setup_e2e_binaries() {
    ...
    export EXTRA_HELM_OPTIONS="--set controller.disableAvailabilitySetNodes=true --set controller.runOnMaster=true --set controller.replicas=1 --set driver.name=$DRIVER.csi.azure.com --set controller.name=csi-$DRIVER-controller --set linux.dsName=csi-$DRIVER-node --set windows.dsName=csi-$DRIVER-node-win --set driver.azureGoSDKLogLevel=DEBUGs --set controller.vmssCacheTTLInSeconds=60"
