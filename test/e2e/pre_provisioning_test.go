...
func CreateVolume(diskName string, sizeGiB int, volumeContext map[string]string) (string, error) {
	...
	diskIOPSReadWriteInt, err := strconv.Atoi(diskIOPSReadWrite)
	if err != nil {
		return "", fmt.Errorf("failed to parse DiskIOPSReadWrite")
	}

	diskMBpsReadWriteInt, err := strconv.Atoi(diskMBpsReadWrite)
	if err != nil {
		return "", fmt.Errorf("failed to parse DiskMBpsReadWrite")
	}
	...
	if err != nil {
		accountType = armcompute.DiskStorageAccountTypesPremiumLRS
	}

	disk := armcompute.Disk{
		Name:			&diskName,
		Properties:		&armcompute.DiskProperties{
			DiskSizeGB: 			to.Ptr(int32(sizeGiB)),
			MaxShares: 				to.Ptr(int32(maxShares)),
			DiskIOPSReadWrite:		to.Ptr(int64(diskIOPSReadWriteInt)),
			DiskMBpsReadWrite:		to.Ptr(int64(diskMBpsReadWriteInt)),
		},
	...
	poller, err := azureCloud.DisksClient.BeginCreateOrUpdate(context.TODO(), azureCloud.ResourceGroup, diskName, disk, nil)
	if err != nil {
		return "", err
	}
...
func EnsureDiskExists(diskURI string) {
	...
	_, err := azureCloud.DisksClient.Get(context.TODO(), azureCloud.ResourceGroup, diskName, nil)
	if err != nil {
