...
var _ = ginkgo.Describe("Controller", func() {
		...
		ginkgo.It("Should create AzDriverNode resource and report heartbeat.", func() {
			...
			for _, pod := range pods.Items {
				if strings.Contains(pod.Spec.NodeName, "csi-azuredisk-node") {
					azN := azDiskClient.DiskV1beta2().AzDriverNodes(namespace)
	...
	ginkgo.Context("AzVolumeAttachment", func() {
		...
		ginkgo.It("Should delete AzVolumeAttachment object properly", func() {
			...
			err = azDiskClient.DiskV1beta2().AzVolumes(namespace).Delete(context.Background(), volName, metav1.DeleteOptions{})
			framework.ExpectNoError(err)

			err = azDiskClient.DiskV1beta2().AzVolumeAttachments(namespace).Delete(context.Background(), att.Name, metav1.DeleteOptions{})
			framework.ExpectNoError(err)
		...
		ginkgo.It("If failover happens, should turn replica to primary and create an additional replica for replacement", func() {
			...
			err := testAzAtt.WaitForReplicas(1, time.Duration(5)*time.Minute)
			framework.ExpectNoError(err)
			attachments, err := azDiskClient.DiskV1beta2().AzVolumeAttachments(namespace).List(context.Background(), metav1.ListOptions{})
			framework.ExpectNoError(err)
			...
			err = azDiskClient.DiskV1beta2().AzVolumeAttachments(namespace).Delete(context.Background(), azureutils.GetAzVolumeAttachmentName(volName, primaryNode), metav1.DeleteOptions{})
			framework.ExpectNoError(err)
			err = testAzAtt.WaitForDelete(primaryNode, time.Duration(5)*time.Minute)
			framework.ExpectNoError(err)

			// failover to one of replicas
			var replica *azdiskv1beta2.AzVolumeAttachment
			for _, attachment := range attachments.Items {
				if attachment.Status.Detail != nil && attachment.Status.Detail.Role == azdiskv1beta2.ReplicaRole {
			...
			promoted.Spec.RequestedRole = azdiskv1beta2.PrimaryRole
			_, err = azDiskClient.DiskV1beta2().AzVolumeAttachments(namespace).Update(context.Background(), promoted, metav1.UpdateOptions{})
			framework.ExpectNoError(err)
		...
		ginkgo.It("If a replica is deleted, should create another replica to replace the previous one", func() {
			...
			err = azDiskClient.DiskV1beta2().AzVolumeAttachments(namespace).Delete(context.Background(), replica.Name, metav1.DeleteOptions{})
			framework.ExpectNoError(err)
		...
		ginkgo.It("number of replicas should converge to assigned maxMountReplicaCount value", func() {
			...
			for _, attachment := range attachments.Items {
				if attachment.Status.Detail != nil && attachment.Status.Detail.Role == azdiskv1beta2.ReplicaRole {
					err = azDiskClient.DiskV1beta2().AzVolumeAttachments(namespace).Delete(context.Background(), attachment.Name, metav1.DeleteOptions{})
					framework.ExpectNoError(err)
					...
					// err = resources.WaitForDelete(azDiskClient.DiskV1beta2().AzVolumeAttachments(namespace), att.Namespace, replica.Name, time.Duration(5)*time.Minute)
					// framework.ExpectNoError(err)
					i++
				}
				if i >= replicaCount {
		...
		ginkgo.It("should garbage collect replica AzVolumeAttachment if no promotion or new primary creation happens in a given period of time", func() {
			...
			err = azDiskClient.DiskV1beta2().AzVolumeAttachments(namespace).Delete(context.Background(), primary.Name, metav1.DeleteOptions{})
			framework.ExpectNoError(err)
			err = testAzAtt.WaitForDelete(primaryNode, time.Duration(5)*time.Minute)
			framework.ExpectNoError(err)
	...
	ginkgo.Context("AzVolumes", func() {
		...
		ginkgo.It("Should delete AzVolume object", func() {
			...
			err = testAzVolume.WaitForFinalizer(time.Duration(5) * time.Minute)
			framework.ExpectNoError(err)
			err = azDiskClient.DiskV1beta2().AzVolumes(namespace).Delete(context.Background(), volume.Name, metav1.DeleteOptions{})
			framework.ExpectNoError(err)
