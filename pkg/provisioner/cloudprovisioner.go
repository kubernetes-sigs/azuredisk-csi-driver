...
type listVolumeStatus struct {
	...
	entries       []azdiskv1beta1.VolumeEntry
	err           error
}
...
func (c *CloudProvisioner) CreateVolume(
	...
	accessibilityRequirements *azdiskv1beta1.TopologyRequirement) (*azdiskv1beta1.AzVolumeStatusDetail, error) {
	var err error
	...
	accessibleTopology := []azdiskv1beta1.Topology{}
	if skuName == compute.DiskStorageAccountTypesStandardSSDZRS || skuName == compute.DiskStorageAccountTypesPremiumZRS {
		...
		for i := 1; i <= 3; i++ {
			topology := azdiskv1beta1.Topology{
	...
	if volumeContentSource != nil {
		...
		if volumeContentSource.ContentSource == azdiskv1beta1.ContentVolumeSourceTypeVolume {
	...
	return &azdiskv1beta1.AzVolumeStatusDetail{
		VolumeID:           diskURI,
...
func (c *CloudProvisioner) ListVolumes(
	ctx context.Context,
	maxEntries int32,
	startingToken string) (*azdiskv1beta1.ListVolumesResult, error) {
...
func (c *CloudProvisioner) CreateSnapshot(
	...
	snapshotName string,
	secrets map[string]string,
	parameters map[string]string) (*azdiskv1beta1.Snapshot, error) {
...
func (c *CloudProvisioner) ListSnapshots(
	...
	sourceVolumeID string,
	snapshotID string,
	secrets map[string]string) (*azdiskv1beta1.ListSnapshotsResult, error) {
	...
	if len(snapshotID) != 0 {
		...
		if err != nil {
			if strings.Contains(err.Error(), azureconstants.ResourceNotFound) {
				return &azdiskv1beta1.ListSnapshotsResult{}, nil
			}
			return nil, err
		}
		entries := []azdiskv1beta1.Snapshot{*snapshot}

		listSnapshotResp := &azdiskv1beta1.ListSnapshotsResult{
			Entries: entries,
		}
	...
	entries := []azdiskv1beta1.Snapshot{}
...
func (c *CloudProvisioner) getSnapshotByID(ctx context.Context, resourceGroup string, snapshotName string, sourceVolumeID string) (*azdiskv1beta1.Snapshot, error) {
...
func (c *CloudProvisioner) validateCreateVolumeRequestParams(
	capacityRange *azdiskv1beta1.CapacityRange,
	volumeCaps []azdiskv1beta1.VolumeCapability,
...
func (c *CloudProvisioner) listVolumesInCluster(ctx context.Context, start, maxEntries int) (*azdiskv1beta1.ListVolumesResult, error) {
	kubeClient := c.cloud.KubeClient
	...
	listVolumesResp := &azdiskv1beta1.ListVolumesResult{
		Entries:   entries,
...
func (c *CloudProvisioner) listVolumesInNodeResourceGroup(ctx context.Context, start, maxEntries int) (*azdiskv1beta1.ListVolumesResult, error) {
	entries := []azdiskv1beta1.VolumeEntry{}
...
func (c *CloudProvisioner) listVolumesByResourceGroup(ctx context.Context, resourceGroup string, entries []azdiskv1beta1.VolumeEntry, start, maxEntries int, volSet map[string]bool) listVolumeStatus {
	...
	for ; i < len(disks); i++ {
		...
		if disk.DiskProperties == nil || disk.DiskProperties.HyperVGeneration == "" {
			...
			entries = append(entries, azdiskv1beta1.VolumeEntry{
				Details: &azdiskv1beta1.VolumeDetails{
					VolumeID: *disk.ID,
				},
				Status: &azdiskv1beta1.VolumeStatus{
...
func pickAvailabilityZone(requirement *azdiskv1beta1.TopologyRequirement, region string) string {
	if requirement == nil {
		return ""
	}
