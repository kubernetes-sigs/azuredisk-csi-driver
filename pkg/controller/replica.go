...
func (r *ReconcileReplica) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if azVolumeAttachment.Spec.RequestedRole == azdiskv1beta1.PrimaryRole {
		...
		if objectDeletionRequested(azVolumeAttachment) {
			...
			if isAttached(azVolumeAttachment) && azVolumeAttachment.Status.Detail.PreviousRole == azdiskv1beta1.ReplicaRole {
		...
		if objectDeletionRequested(azVolumeAttachment) {
			...
			case azdiskv1beta1.DetachmentFailed:
				if err := azureutils.UpdateCRIWithRetry(ctx, nil, r.controllerSharedState.cachedClient, r.controllerSharedState.azClient, azVolumeAttachment, func(obj interface{}) error {
					azVolumeAttachment := obj.(*azdiskv1beta1.AzVolumeAttachment)
					_, err = updateState(azVolumeAttachment, azdiskv1beta1.ForceDetachPending, normalUpdate)
					return err
		...
		} else if azVolumeAttachment.Status.State == azdiskv1beta1.AttachmentFailed {
...
func NewReplicaController(mgr manager.Manager, controllerSharedState *SharedState) (*ReconcileReplica, error) {
	...
	err = c.Watch(&source.Kind{Type: &azdiskv1beta1.AzVolumeAttachment{}}, &handler.EnqueueRequestForObject{}, predicate.Funcs{
		CreateFunc: func(e event.CreateEvent) bool {
			azVolumeAttachment, ok := e.Object.(*azdiskv1beta1.AzVolumeAttachment)
			if ok && azVolumeAttachment.Spec.RequestedRole == azdiskv1beta1.PrimaryRole {
				return true
			}
			return false
		},
