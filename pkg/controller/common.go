...
var roles = map[roleMode]string{
	primaryOnly: string(azdiskv1beta1.PrimaryRole),
	replicaOnly: string(azdiskv1beta1.ReplicaRole),
}
...
type CloudProvisioner interface {
	...
	ListVolumes(ctx context.Context, maxEntries int32, startingToken string) (*azdiskv1beta1.ListVolumesResult, error)
	CreateSnapshot(ctx context.Context, sourceVolumeID string, snapshotName string, secrets map[string]string, parameters map[string]string) (*azdiskv1beta1.Snapshot, error)
	ListSnapshots(ctx context.Context, maxEntries int32, startingToken string, sourceVolumeID string, snapshotID string, secrets map[string]string) (*azdiskv1beta1.ListSnapshotsResult, error)
	...
	for _, volume := range s.volumes {
		...
		for _, azVolumeAttachment := range azVolumeAttachments {
			if score, exists := nodeScores[azVolumeAttachment.Spec.NodeName]; exists {
				if azVolumeAttachment.Spec.RequestedRole == azdiskv1beta1.PrimaryRole {
...
func (c *SharedState) createReplicaAzVolumeAttachment(ctx context.Context, volumeID, node string, volumeContext map[string]string) error {
	...
	azVolumeAttachment := azdiskv1beta1.AzVolumeAttachment{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				...
				consts.RoleLabel:       string(azdiskv1beta1.ReplicaRole),
			},
	...
	_, err = c.azClient.DiskV1beta1().AzVolumeAttachments(c.objectNamespace).Create(ctx, &azVolumeAttachment, metav1.CreateOptions{})
	if err != nil {
...
func (c *SharedState) cleanUpAzVolumeAttachmentByVolume(ctx context.Context, azVolumeName string, caller operationRequester, role roleMode, deleteMode cleanUpMode) ([]azdiskv1beta1.AzVolumeAttachment, error) {
	var err error
...
func (c *SharedState) cleanUpAzVolumeAttachmentByNode(ctx context.Context, azDriverNodeName string, caller operationRequester, role roleMode, deleteMode cleanUpMode) ([]azdiskv1beta1.AzVolumeAttachment, error) {
	var err error
	...
	var attachments *azdiskv1beta1.AzVolumeAttachmentList
	...
	cleanUpMap := map[string][]azdiskv1beta1.AzVolumeAttachment{}
...
func (c *SharedState) cleanUpAzVolumeAttachments(ctx context.Context, attachments []azdiskv1beta1.AzVolumeAttachment, cleanUp cleanUpMode, caller operationRequester) error {
	var err error
	w, _ := workflow.GetWorkflowFromContext(ctx)
	...
	for _, attachment := range attachments {
		...
		if (caller != azdrivernode && !metav1.HasAnnotation(patched.ObjectMeta, consts.CleanUpAnnotation)) ||
			...
			((cleanUp == detachAndDeleteCRI || patched.Spec.RequestedRole == azdiskv1beta1.ReplicaRole) && !metav1.HasAnnotation(patched.ObjectMeta, consts.VolumeDetachRequestAnnotation)) {
			patchRequired = true
			...
			if cleanUp == detachAndDeleteCRI || patched.Spec.RequestedRole == azdiskv1beta1.ReplicaRole {
...
func getAzVolumeAttachmentsForVolume(ctx context.Context, azclient client.Client, volumeName string, azVolumeAttachmentRole roleMode) (attachments []azdiskv1beta1.AzVolumeAttachment, err error) {
	w, _ := workflow.GetWorkflowFromContext(ctx)
...
func getAzVolumeAttachmentsForNode(ctx context.Context, azclient client.Client, nodeName string, azVolumeAttachmentRole roleMode) (attachments []azdiskv1beta1.AzVolumeAttachment, err error) {
	w, _ := workflow.GetWorkflowFromContext(ctx)
...
func getAzVolumeAttachmentsWithLabel(ctx context.Context, azclient client.Client, labelPairs ...labelPair) (attachments []azdiskv1beta1.AzVolumeAttachment, err error) {
	w, _ := workflow.GetWorkflowFromContext(ctx)
	...
	azVolumeAttachments := &azdiskv1beta1.AzVolumeAttachmentList{}
...
func shouldCleanUp(attachment azdiskv1beta1.AzVolumeAttachment, mode roleMode) bool {
	return mode == all || (attachment.Spec.RequestedRole == azdiskv1beta1.PrimaryRole && mode == primaryOnly) || (attachment.Spec.RequestedRole == azdiskv1beta1.ReplicaRole && mode == replicaOnly)
}

func isAttached(attachment *azdiskv1beta1.AzVolumeAttachment) bool {
	return attachment != nil && attachment.Status.Detail != nil && attachment.Status.Detail.PublishContext != nil
}

func isCreated(volume *azdiskv1beta1.AzVolume) bool {
...
func isCleanupRequested(attachment *azdiskv1beta1.AzVolumeAttachment) bool {
	return attachment != nil && azureutils.MapContains(attachment.Status.Annotations, consts.CleanUpAnnotation)
}

func volumeDetachRequested(attachment *azdiskv1beta1.AzVolumeAttachment) bool {
	return attachment != nil && azureutils.MapContains(attachment.Status.Annotations, consts.VolumeDetachRequestAnnotation)
}

func volumeDeleteRequested(volume *azdiskv1beta1.AzVolume) bool {
	return volume != nil && azureutils.MapContains(volume.Status.Annotations, consts.VolumeDeleteRequestAnnotation)
}

func isDemotionRequested(attachment *azdiskv1beta1.AzVolumeAttachment) bool {
	return attachment != nil && attachment.Status.Detail != nil && attachment.Status.Detail.Role == azdiskv1beta1.PrimaryRole && attachment.Spec.RequestedRole == azdiskv1beta1.ReplicaRole
}

func isPreProvisionCleanupRequested(volume *azdiskv1beta1.AzVolume) bool {
...
func isOperationInProcess(obj interface{}) bool {
	...
	case *azdiskv1beta1.AzVolume:
		return target.Status.State == azdiskv1beta1.VolumeCreating || target.Status.State == azdiskv1beta1.VolumeDeleting || target.Status.State == azdiskv1beta1.VolumeUpdating
	case *azdiskv1beta1.AzVolumeAttachment:
		return target.Status.State == azdiskv1beta1.Attaching || target.Status.State == azdiskv1beta1.Detaching
	}
	return false
}
...
func (c *SharedState) garbageCollectReplicas(ctx context.Context, volumeName string, requester operationRequester) {
	c.addToOperationQueue(
		...
		replica,
		func(ctx context.Context) error {
			if _, err := c.cleanUpAzVolumeAttachmentByVolume(ctx, volumeName, requester, replicaOnly, detachAndDeleteCRI); err != nil {
				return err
			}
...
func (c *SharedState) manageReplicas(ctx context.Context, volumeName string) error {
	...
	if desiredReplicaCount > currentReplicaCount {
		...
		if azVolume.Status.Detail == nil || azVolume.Status.State == azdiskv1beta1.VolumeDeleting || azVolume.Status.State == azdiskv1beta1.VolumeDeleted {
