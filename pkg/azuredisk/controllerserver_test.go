	...
	testCases := []struct {
		...
		},
		{
			...
			testFunc: func(t *testing.T) {
				...
				_, err := d.CreateVolumeGroupSnapshot(context.Background(), req)
				expectedErr := status.Errorf(codes.Internal, "AzureDisk - invalid option unit-test in VolumeGroupSnapshotClass")
				if !reflect.DeepEqual(err, expectedErr) {
					t.Errorf("actualErr: (%v), expectedErr: (%v)", err, expectedErr)
				}
			},
	...
	testCases := []struct {
		...
		},
		{
			...
			testFunc: func(t *testing.T) {
				...
				defer cntl.Finish()
				d, _ := NewFakeDriver(cntl)
				expectedresponse := &csi.GetVolumeGroupSnapshotResponse{}
				actualresponse, err := d.GetVolumeGroupSnapshot(context.Background(), req)
				if !reflect.DeepEqual(expectedresponse, actualresponse) || err != nil {
					t.Errorf("actualresponse: (%+v), expectedresponse: (%+v)\n", actualresponse, expectedresponse)
					t.Errorf("err:%v", err)
				}
