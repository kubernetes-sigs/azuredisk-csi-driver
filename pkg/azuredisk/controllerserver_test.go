		...
		if test.simulateRestart {
			...
			diskclientForRestart.EXPECT().Get(gomock.Any(), gomock.Any(), gomock.Any()).DoAndReturn(
				func(_ context.Context, _, _ string) (*armcompute.Disk, error) {
					...
					diskCopy.Properties.CompletionPercent = to.Ptr(float32(diskCompletionPercent))
					return diskCopy, nil
				},
			).AnyTimes()
			...
			diskCompletionPercent = 100

			// Wait for active migrations to become zero maximum 2 seconds
			...
			// Wait for all active migrations to complete
			for {
				activeMigrations := drestart.GetMigrationMonitor().GetActiveMigrations()
