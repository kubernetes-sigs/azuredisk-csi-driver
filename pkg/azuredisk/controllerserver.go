...
func (d *Driver) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	for key, value := range diskParams.Tags {
		tags[key] = &value
	}

	iops, _ := strconv.Atoi(diskParams.DiskIOPSReadWrite)
	mbps, _ := strconv.Atoi(diskParams.DiskMBPSReadWrite)

	encryptionType := armcompute.EncryptionType(diskParams.DiskEncryptionType)

	creationData, err := azureutils.GetValidCreationData(diskParams.SubscriptionID, diskParams.ResourceGroup, sourceID, sourceType)
	...
	disk := armcompute.Disk{
		...
		Name: &diskParams.DiskName,
		Tags: tags,
		Properties: &armcompute.DiskProperties{
			CreationData:      &creationData,
			DiskSizeGB:        pointer.Int32(int32(requestGiB)),
			DiskIOPSReadWrite: pointer.Int64(int64(iops)),
			DiskMBpsReadWrite: pointer.Int64(int64(mbps)),
			MaxShares:         pointer.Int32(int32(diskParams.MaxShares)),
...
func (d *Driver) ControllerExpandVolume(ctx context.Context, req *csi.ControllerExpandVolumeRequest) (*csi.ControllerExpandVolumeResponse, error) {
	...
	// subsID := azureutils.GetSubscriptionIDFromURI(diskURI)
	resp, rerr := d.cloud.DisksClient.Get(ctx, resourceGroup, diskName, nil)
	result := resp.Disk
	if rerr != nil {
...
func (d *Driver) CreateSnapshot(ctx context.Context, req *csi.CreateSnapshotRequest) (*csi.CreateSnapshotResponse, error) {
	...
	if _, rerr := d.cloud.SnapshotsClient.BeginCreateOrUpdate(ctx, resourceGroup, snapshotName, snapshot, nil); rerr != nil {
...
func (d *Driver) getSnapshotByID(ctx context.Context, subsID, resourceGroup, snapshotID, sourceVolumeID string) (*csi.Snapshot, error) {
	...
	resp, rerr := d.cloud.SnapshotsClient.Get(ctx, resourceGroup, snapshotName, nil)
...
func (d *Driver) GetSourceDiskSize(ctx context.Context, subsID, resourceGroup, diskName string, curDepth, maxDepth int) (*int32, error) {
	...
	result, rerr := d.cloud.DisksClient.Get(ctx, resourceGroup, diskName, nil)
	if rerr != nil {
		return nil, rerr
	}
	...
	if result.Properties.CreationData != nil && (*result.Properties.CreationData).CreateOption == to.Ptr(armcompute.DiskCreateOptionCopy) {
...
func (d *Driver) getCloudProviderWithUserAgent(ctx context.Context, userAgent string) (*azureutils.Cloud, error) {
