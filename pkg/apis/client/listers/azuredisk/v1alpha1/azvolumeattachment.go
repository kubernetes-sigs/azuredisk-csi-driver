...
type AzVolumeAttachmentLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.AzVolumeAttachment, err error)
...
// List lists all AzVolumeAttachments in the indexer.
func (s *azVolumeAttachmentLister) List(selector labels.Selector) (ret []*v1alpha1.AzVolumeAttachment, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.AzVolumeAttachment))
	})
	return ret, err
}
...
type AzVolumeAttachmentNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.AzVolumeAttachment, error)
...
// List lists all AzVolumeAttachments in the indexer for a given namespace.
func (s azVolumeAttachmentNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.AzVolumeAttachment, err error) {
...
// Get retrieves the AzVolumeAttachment from the indexer for a given namespace and name.
func (s azVolumeAttachmentNamespaceLister) Get(name string) (*v1alpha1.AzVolumeAttachment, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("azvolumeattachment"), name)
	}
	return obj.(*v1alpha1.AzVolumeAttachment), nil
}
