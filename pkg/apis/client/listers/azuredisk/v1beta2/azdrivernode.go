...
type AzDriverNodeLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta2.AzDriverNode, err error)
...
// List lists all AzDriverNodes in the indexer.
func (s *azDriverNodeLister) List(selector labels.Selector) (ret []*v1beta2.AzDriverNode, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta2.AzDriverNode))
	})
	return ret, err
}
...
type AzDriverNodeNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta2.AzDriverNode, error)
...
// List lists all AzDriverNodes in the indexer for a given namespace.
func (s azDriverNodeNamespaceLister) List(selector labels.Selector) (ret []*v1beta2.AzDriverNode, err error) {
...
// Get retrieves the AzDriverNode from the indexer for a given namespace and name.
func (s azDriverNodeNamespaceLister) Get(name string) (*v1beta2.AzDriverNode, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta2.Resource("azdrivernode"), name)
	}
	return obj.(*v1beta2.AzDriverNode), nil
}
