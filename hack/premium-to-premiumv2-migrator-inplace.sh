...
# It uses batch cache for optimized lookups and supports MODE=inplace
...
for ENTRY in "${SOURCE_SNAPSHOTS[@]}"; do
  ...
  # Use cached PVC JSON to get PV name (cache still valid before create_pvc_from_snapshot)
  pvc_json=$(get_cached_pvc_json "$pvc_ns" "$pvc")
  pv=$(echo "$pvc_json" | jq -r '.spec.volumeName // empty')
...
for ENTRY in "${SOURCE_SNAPSHOTS[@]}"; do
  ...
  if wait_pvc_bound "$pvc_ns" "$pvc" "$BIND_TIMEOUT_SECONDS"; then
    ...
    audit_add "PersistentVolumeClaim" "$pvc" "$pvc_ns" "bound" "kubectl describe pvc $pvc -n $pvc_ns" "sc=${sc}"
    continue
  fi

  warn "PVC $pvc_ns/$pvc not bound within timeout (${BIND_TIMEOUT_SECONDS}s)"
  audit_add "PersistentVolumeClaim" "$pvc" "$pvc_ns" "bind-timeout" "kubectl describe pvc $pvc -n $pvc_ns" "sc=${sc} timeout=${BIND_TIMEOUT_SECONDS}s"
...
while true; do
  ...
  for ENTRY in "${MIG_PVCS[@]}"; do
    ...
    # Fetch PVC JSON once per iteration and extract all needed fields
    pvc_json=$(kcmd get pvc "$pvc" -n "$pvc_ns" -o json 2>/dev/null || true)
    [[ -z "$pvc_json" ]] && { warn "Could not fetch PVC $pvc_ns/$pvc"; continue; }
    ...
    STATUS=$(echo "$pvc_json" | jq -r '.status.phase // empty')
    ...
    if [[ "$STATUS" == "Bound" && -z "$reason" && $MIGRATION_FORCE_INPROGRESS_AFTER_MINUTES -gt 0 ]]; then
      orig_pv=$(echo "$pvc_json" | jq -r '.spec.volumeName // empty')
      [[ -z "$orig_pv" ]] && continue
